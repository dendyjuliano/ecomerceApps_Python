...
class CartAndItemModelsTestCase(TestCase):
    ...
    def _create_cart_in_database(self, creation_date=datetime.datetime.now(), checked_out=False):
        cart = models.Cart.objects.create(
            creation_date=creation_date, checked_out=checked_out)
        return cart
    ...
    def _create_user_in_database(self):
        user = User.objects.create(
            username="user_for_sell", password="sold", email="example@example.com")
        return user
    ...
    def test_item_creation_and_association_with_cart(self):
        ...
        item = self._create_item_in_database(
            cart, user, quantity=1, unit_price=Decimal("100"))

        # get the first item in the cart
        item_in_cart = cart.item_set.all()[0]
        self.assertEquals(
            item_in_cart, item, "First item in cart should be equal the item we created")
    ...
    def test_total_item_price(self):
        ...
        self.assertEquals(
            item_with_unit_price_as_decimal.total_price, Decimal("12.80"))
    ...
    def test_item_unicode(self):
        ...
        item = self._create_item_in_database(
            cart, product=user, quantity=3, unit_price=Decimal("100"))
